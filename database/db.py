...
class Database:
    ...
    async def reset_fwd_settings(self, id):
        ...
        Rexbots features, not just forwarding — /logout handles that."""
    ...
    # (/channels, /addroute, /delroute — Rexbots/channels.py)
    #
    ...
    # --------------------------------------------------------
    # Per-user MongoDB (/set_mydb, /del_mydb, /mydb — Rexbots/userdb.py)
    #
    # Rexbots itself always keeps running on the shared DB above (DB_URI/
...
def _extract_db_name(uri: str) -> str:
    ...
    try:
        ...
        return name or "Rexbots_userdb"
    except Exception:
        return "Rexbots_userdb"
...
# forward-bot's "FTM Manager" (gamma source/target, replacer/remover/
...
# ------ Extra FTM Manager toggles: numbering, bullets, username/link
