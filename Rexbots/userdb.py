...
# string instead of relying only on Rexbots' one shared DB (config.DB_URI /
# config.DB_NAME). Rexbots itself never stops using the shared DB — that's
...
# visible effect yet on its own. It's the foundation the Channels/Bots/FTM
...
try:
    from Rexbots.settings import make_button, get_back_close_buttons, BUTTON_STYLE_SUPPORTED, ICON_INFO
...
def _status_text(uri: str | None) -> str:
    ...
    return (
        f"<b>{E_INFO} Your MongoDB</b>\n\n"
        f"<b>Status:</b> {E_CROSS} not set — using Rexbots' shared DB\n\n"
...
async def set_mydb_cmd(client: Client, message: Message):
    ...
    try:
        ...
        return await status.edit_text(
            ...
            f"connections from anywhere (0.0.0.0/0) — Rexbots' server IP isn't fixed.</i>",
            parse_mode=enums.ParseMode.HTML
        )
...
async def del_mydb_cmd(client: Client, message: Message):
    ...
    await message.reply_text(
        f"<b>{E_CHECK} Disconnected.</b> Falling back to Rexbots' shared DB.",
        parse_mode=enums.ParseMode.HTML
    )
