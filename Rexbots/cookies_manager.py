...
async def setcookies_file_receive(client: Client, message: Message):
    ...
    if panel_pending:
        ...
        note = f"\n<i>Other domains also seen in this file: {', '.join(others)}</i>" if others else ""
        await message.reply_text(
            f"<b>{E_CHECK} Success!</b>\n\n"
            f"Cookies automatically assigned to domain: <code>{domain}</code> "
            f"({all_counts[domain]} cookie(s))"
            f"{note}\n\n"
            f"<i>Wrong site? Re-upload with </i><code>/setcookies correct-domain.com</code><i> instead.</i>",
            parse_mode=enums.ParseMode.HTML
        )
        message.stop_propagation()
        return
