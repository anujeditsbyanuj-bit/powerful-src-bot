...
#      shared Rexbots one. It reads/writes the exact same per-user data
...
# plugins=dict(root="Rexbots") — that auto-loads and attaches EVERY
# handler in this package to whatever Client instantiates it, with none
# of them owner-scoped (a lot of Rexbots' ~50 other plugins only check
...
# Rexbots later is possible, but each addition should get the same
# "is this actually safe to expose owner-scoped, unaudited" look this
# list got — don't just point it at plugins=dict(root="Rexbots").
#
...
async def _attach_personal_bot(clone: Client, owner_id: int, token: str):
    ...
    # forward.py imports `from Rexbots import titanium` for get_job_client.
    from Rexbots import forward as _fwd
    from Rexbots import channels as _chan
...
async def _personal_start_cmd(client: Client, message: Message):
    ...
    await message.reply_text(
        f"<b>{E_CHECK} @{me.username} is your personal Rexbots forward-bot.</b>\n\n"
        f"It shares your saved settings with the main Rexbots bot — /fwd, /setsource, "
        f"/settarget, /channels, /fwd_settings all work here the same way, just on your "
        f"own bot token.\n\n"
        f"<i>Manage this connection (or add more) from the main Rexbots bot with /titanium.</i>",
