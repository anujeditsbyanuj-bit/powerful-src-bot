# Rexbots
# FTM Manager menu — mirrors the reference forward-bot's /start layout
# (Bots / Channels / Caption / MongoDB / Filters / Button / FTM Manager /
# Extra Settings / Back) using Rexbots' own existing features wherever one
# already exists, and clearly marking the pieces that are still pending
# the bigger forwarding-engine port (Filters, Button, core FTM Manager).
#
# Don't Remove Credit
...
def _ftm_manager_menu() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup([
        [InlineKeyboardButton("🤖 Bots", callback_data="ftmm_bots"),
        ...
        [InlineKeyboardButton("🕵️ Filters", callback_data="ftmm_filters"),
         InlineKeyboardButton("🔘 Button", callback_data="ftmm_button")],
        [InlineKeyboardButton("🚀 FTM Manager 🚀", callback_data="ftmm_core")],
        [InlineKeyboardButton("🧪 Extra Settings", callback_data="ftmm_extra")],
        [InlineKeyboardButton(f"{E_BACK} Back", callback_data="settings_back_btn")],
    ])


@Client.on_message(filters.command("ftmmanager") & filters.private)
async def ftmmanager_command(client: Client, message: Message):
    if not await db.is_user_exist(message.from_user.id):
        await db.add_user(message.from_user.id, message.from_user.first_name)
    await message.reply_text(
        f"<b>🚀 FTM Manager</b>\n\n<i>Change your settings as you wish:</i>",
        reply_markup=_ftm_manager_menu(),
        parse_mode=enums.ParseMode.HTML,
    )


@Client.on_callback_query(filters.regex("^ftmmanager_btn$"))
async def ftmmanager_btn_callback(client: Client, callback_query: CallbackQuery):
...
@Client.on_callback_query(filters.regex("^ftmm_bots$"))
async def ftmm_bots_callback(client: Client, callback_query: CallbackQuery):
    await callback_query.answer()
    await callback_query.message.edit_text(
        f"<blockquote>{E_INFO} <b>Bots</b>\n\n"
        f"<code>/setbot &lt;token&gt;</code> — Connect your own bot to run alongside Rexbots\n"
        f"<code>/rembot</code> — Disconnect your bot</blockquote>",
        reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton(f"{E_BACK} Back", callback_data="ftmmanager_btn")]]),
        parse_mode=enums.ParseMode.HTML,
    )


@Client.on_callback_query(filters.regex("^ftmm_extra$"))
async def ftmm_extra_callback(client: Client, callback_query: CallbackQuery):
    await callback_query.answer()
    await callback_query.message.edit_text(
...
@Client.on_callback_query(filters.regex("^ftmm_filters$"))
async def ftmm_filters_callback(client: Client, callback_query: CallbackQuery):
    await callback_query.answer()
    from Rexbots.forward_engine import FILTER_TYPES
    await callback_query.message.edit_text(
...
@Client.on_callback_query(filters.regex("^ftmm_button$"))
async def ftmm_button_callback(client: Client, callback_query: CallbackQuery):
    await callback_query.answer()
    await callback_query.message.edit_text(
...
@Client.on_callback_query(filters.regex("^ftmm_core$"))
async def ftmm_core_callback(client: Client, callback_query: CallbackQuery):
    await callback_query.answer()
    await callback_query.message.edit_text(
        f"<blockquote>{E_INFO} <b>FTM Manager (Forwarding Core)</b>\n\n"
