...
@Client.on_message(filters.private & filters.command(["login", "fwd_login"]))
...
async def _login_cancel_matches(_, __, message: Message) -> bool:
    """/cancellogin always belongs to this handler. Bare /cancel is
    ambiguous — start.py's send_cancel() also listens for it (batch/
    download cancellation), and Pyrogram only runs the FIRST matching
    handler within a group. Previously this handler always matched /cancel
    unconditionally and silently did nothing when the user wasn't mid-
    login (the `else: pass` below), which meant start.py's handler never
    even got a chance to run — /cancel looked completely broken for the
    (far more common) case of cancelling a download instead of a login.
    Only claim /cancel here when there's actually an active login flow to
    cancel; otherwise let the message fall through to start.py's handler."""
...
login_cancel_filter = filters.create(_login_cancel_matches)


@Client.on_message(filters.private & login_cancel_filter)
